...
export function register(config) {
    if (process.env.NODE_ENV === "production" && "serviceWorker" in navigator) {
        ...
        const publicUURL = new URL(process.env.PUBLIC_URL, window.location.href);
        if (publicUURL.origin !== window.location.origin)
...
function registerValidServiceWorker(serviceWorkerURL, config) {
    navigator.serviceWorker.register(serviceWorkerURL).then(registration => registration.onupdatefound = () => {
